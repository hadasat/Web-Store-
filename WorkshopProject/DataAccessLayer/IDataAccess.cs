...
namespace WorkshopProject.DataAccessLayer
{
    public interface IDataAccess
    {
        ...
        bool RemoveEntity<T>(int key) where T : class, IEntity;
        bool SaveEntity<T>(IEntity entity, int key) where T : class, IEntity;
        void SetMode(bool isProduction);
        DbRawSqlQuery<T> SqlQuery<T>(string sql, params object[] paramaters) where T : class, IEntity;
    }
}